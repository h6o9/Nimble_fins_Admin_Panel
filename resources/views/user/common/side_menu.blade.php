<div class="main-sidebar sidebar-style-2">
    <aside id="sidebar-wrapper">
        <div class="sidebar-brand">
            {{-- <svg style="width: 80%" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
            ...
            <a href="{{url('user/dashboard')}}"><img src="https://res.cloudinary.com/nimblefins/image/upload/c_limit,dpr_1.0,f_auto,h_1600,q_auto,w_1600/v1/UK/static/nimblefins-logo-png" class="admin-logo" /></a>
        </div>
        <ul class="sidebar-menu">
            <li class="dropdown {{ request()->is('user/dashboard') ? 'active' : '' }}">
                <a href="{{ url('/user/dashboard') }}" class="nav-link">
                    <img src="{{asset('public/user/icons/overview.png')}}" style="filter: invert(1);width: 35px;padding-right: 5px" /><span>Overview</span></a>
            </li>  
            <li class="dropdown {{ request()->is('user/profile') ? 'active' : '' }}">
                <a href="{{ route('user.profile') }}" class="nav-link">
                    <img src="{{asset('public/user/icons/profile.svg')}}" style="filter: invert(1);width: 35px;padding-right: 5px" /> <span>Profile</span>
                </a>
            </li>
            <li class="dropdown {{ request()->is('user/certificates*') ? 'active' : '' }}">
                <a href="{{ route('user.certificates') }}" class="nav-link">
                    <img src="{{asset('public/user/icons/purchases.svg')}}" style="filter: invert(1);width: 35px;padding-right: 5px" /><span>Purchases</span>
                </a>
            </li> 
            <li class="dropdown {{ request()->is('user/marketing*') ? 'active' : '' }}">
                <a href="{{route('marketing')}}" class="nav-link">
                    <img src="{{asset('public/user/icons/marketing.svg')}}" style="filter: invert(1);width: 35px;padding-right: 5px" /><span>Marketing Preferences</span>
                </a>
            </li>
            <li class="dropdown {{ request()->is('user/404/two*') ? 'active' : '' }}">
                <a href="{{route('error.page2')}}" class="nav-link">
                    <img src="{{asset('public/user/icons/referrals.svg')}}" style="filter: invert(1);width: 35px;padding-right: 5px" /><span>Referrals</span>
                </a>
            </li>
        </ul>
    </aside>
